...
var MailService = function () {
    ...
    var MailService = _classThis = /** @class */ (function () {
        function MailService_1(config, jwtService) {
            this.config = config;
         ...
         */
        MailService_1.prototype.generateVerificationUrl = function (userId, email) {
            var token = this.jwtService.sign({ sub: userId, email: email, type: 'email-verification' }, { expiresIn: '24h' });
            var appUrl = this.config.get('APP_URL', 'http://localhost:3000');
            return "".concat(appUrl, "/email/verify?token=").concat(token);
        };
        MailService_1.prototype.sendVerificationEmail = function (userId, email, name) {
            return __awaiter(this, void 0, void 0, function () {
                var verificationUrl, fromName, fromAddress, err_1;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            verificationUrl = this.generateVerificationUrl(userId, email);
                            fromName = this.config.get('MAIL_FROM_NAME', 'Redis NestJS App');
                            fromAddress = this.config.get('MAIL_FROM', 'noreply@example.com');
                            _a.label = 1;
                        case 1:
                            _a.trys.push([1, 3, , 4]);
                        ...
                        case 2:
                            _a.sent();
                            this.logger.log("Verification email sent to ".concat(email));
                            return [3 /*break*/, 4];
                        case 3:
                            err_1 = _a.sent();
                            this.logger.error("Failed to send email to ".concat(email, ": ").concat(err_1.message));
                            throw err_1;
